...
class UpdaterServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->singleton(LoggerInterface::class, fn () => LoggerFactory::make(config('updater.log')));

        $this->app->singleton(PreflightChecker::class, fn () => new PreflightChecker(
            $this->app->make(ShellRunner::class),
            $this->app->make(CodeDriverInterface::class),
            config('updater.preflight')
        ));

        $this->app->singleton(TriggerDispatcher::class, fn () => new TriggerDispatcher(
    ...
    public function boot(Router $router): void
    {
        ...
        $this->app->make(AuthStore::class)->ensureAdminProvisioned(config('updater.ui.auth', []));
