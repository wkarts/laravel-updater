...
class OperationsController extends Controller
{
    ...
    public function triggerDryRun(TriggerDispatcher $dispatcher): RedirectResponse
    {
        ...
        if (!(bool) ($diagnostic['ok'] ?? false)) {
            return back()->withErrors(['dry_run' => (string) ($diagnostic['reason'] ?? 'Execução web indisponível no momento.')]);
        }
    ...
    public function downloadRunLogReport(Request $request)
    {
        $runId = $request->filled('run_id') ? (int) $request->input('run_id') : null;
        $run = $runId !== null ? $this->stateStore->findRun($runId) : $this->stateStore->lastRun();

        $logs = $this->managerStore->logs($runId, $request->input('level'), $request->input('q'));
        ...
        $filename = 'updater-report-' . ($run['id'] ?? 'latest') . '-' . date('Ymd-His') . '.json';
